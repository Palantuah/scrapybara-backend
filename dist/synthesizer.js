...
const CSV_FILE = path_1.default.join(__dirname, '..', 'email_database.csv');
const REPORTS_DIR = path_1.default.join(__dirname, '..', 'outputs', 'category_reports');
...
let lastCsvHash = "";
async function generateNewsletter(category, content, existingData) {
    try {
...
DO NOT include information from other sectors or categories unless it directly impacts ${category}.`;
        ...
        const { text: keywordsText } = await (0, ai_1.generateText)({
            model: (0, openai_1.openai)('gpt-4o-mini'),
            messages: [
...
async function updateCategoryNewsletter(category, content) {
    ...
    try {
        if (fs_1.default.existsSync(filePath)) {
            ...
            existingData = JSON.parse(fs_1.default.readFileSync(filePath, 'utf-8'));
...
function computeHash(content) {
    return crypto_1.default.createHash('md5').update(content.trim()).digest('hex');
}
async function processCsvFile() {
    try {
        if (!fs_1.default.existsSync(CSV_FILE)) {
            console.log('CSV file not found. Waiting for it to be created...');
            lastProcessedLine = 0;
            return;
        }
        const fileContent = fs_1.default.readFileSync(CSV_FILE, 'utf-8');
        ...
        const records = (0, sync_1.parse)(csvToParse, {
            columns: true,
            skip_empty_lines: true
        });
        for (const record of records) {
            if (record['Subject'] === 'Subject')
                continue; // skip header if parsed as record
